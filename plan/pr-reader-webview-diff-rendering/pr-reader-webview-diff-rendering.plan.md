...
不實作任何 parser、renderer、DOM、UI、collapse、syntax highlighting、Swift bridge、GitHub mapping 或 viewed-state 持久化；不新增 package 或修改 lockfile。`viewed` notification 不新增 outcome、reliable transport、acknowledgement、retry 或 bridge implementation。僅回寫本 topic 已鎖定的 PR Reader WebView diff pipeline 長期責任，不重開或變更 PR Reader BC 邊界。
...
- `docs/architecture/README.md` 與 PR Reader BC 文件中已確認的 snapshot、render flow 與 viewed notification exception 事實。
- `docs/architecture/diagrams/pr-reader-webview-diff-rendering/` 下的更新後 scene 與 generated index，以及 architecture-canvas validate／build evidence。

## Modify

僅可修改本 topic 的四份 artifacts，以及 Implementer 為落實已鎖定 contract 所需的 TypeScript declarations、tests、`docs/architecture/README.md`、PR Reader BC 文件與既有 PR Reader WebView diff architecture-canvas scene／generated index。移除既有集中式 `src/diff-rendering.contract.ts` 與其 test，替換為已鎖定的 `src/diff-rendering/` tree。除已確認的架構 writeback 與 canvas alignment 外，其他現有 repository 檔案不修改。

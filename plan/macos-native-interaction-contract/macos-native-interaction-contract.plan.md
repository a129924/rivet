...
- Current status：原topic維持completed／adopted。上一輪PR-12 `approved`、IM-08 completed、TE-10 PASS、RV-14 `approved`、Human commit message confirmed、DL-11 commit `d209c61`、DL-12 push與DL-13四threads reply＋resolve均已完成。PC-13 entry baseline的PR head、local HEAD與origin head皆為`d209c61`且working tree clean。本輪4個M1～M4 threads均unresolved／non-outdated；RV-15已明示`needs-rework`並鎖定M1～M4。PC-13已完成planning correction，下一gate唯一為獨立PR-13；IM-09／TE-11／RV-16／DL-14～DL-16均pending。此cycle不授權或自動開始Swift implementation。
...
| PC-11 T1～T6 planning／technical correction（completed） | `analysis/macos-native-interaction-contract/technical-spec.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.plan.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.step.md` |
...
| PC-12 L1～L4 planning／technical correction（completed） | `analysis/macos-native-interaction-contract/technical-spec.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.plan.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.step.md` |
| PR-12 planning verification | `None` |
| IM-08 L1／L2／L4 durable correction（只可在PR-12 `approved`後） | `docs/presentation/native-interaction-contract.md` |
| TE-10／RV-14 verification | `None` |
| DL-11 human-confirmed commit／DL-12 push／DL-13 four-thread resolution | `None` |
| PC-13 M1～M4 planning／technical correction（current） | `analysis/macos-native-interaction-contract/technical-spec.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.plan.md`<br>`plan/macos-native-interaction-contract/macos-native-interaction-contract.step.md` |
| PR-13 planning verification | `None` |
| IM-09 M2～M4 durable correction（只可在PR-13 `approved`後） | `docs/presentation/native-interaction-contract.md` |
| TE-11／RV-16 verification | `None` |
| DL-14 human-confirmed commit／DL-15 push／DL-16 four-thread resolution | `None` |
...
- PC-11的`Written`與`Deleted`均為`None`，`Modify`只包含technical spec、plan與step ledger三個exact paths；PR-11 `approved`後IM-07只修改durable contract，該cycle已完成delivery。
- PC-12的`Written`與`Deleted`均為`None`，`Modify`只包含technical spec、plan與step ledger三個exact paths；PR-12 `approved`後IM-08只修改durable contract，該cycle已完成delivery。
- Current PC-13的`Written`與`Deleted`均為`None`，`Modify`只包含technical spec、plan與step ledger三個exact paths；requirements、durable contract及所有未列paths均ReadOnly。只有獨立PR-13明示`approved`後，IM-09的`Modify`才只包含`docs/presentation/native-interaction-contract.md`；其`Written`／`Deleted`仍為`None`，其他paths全為ReadOnly。
...
### IM-08.1 — 套用L1／L2／L4 bounded durable corrections

- 只修改durable output；不得修改planning artifacts、requirements、prototype、Swift或其他path。L3（thread `PRRT_kwDOUFu0Cc6jPcJM`）只屬workflow truth同步，不重跑PR-11～DL-10。
- L1（thread `PRRT_kwDOUFu0Cc6jPcJD`）：非modal Reader `error`／`offline-without-readable-content`時Back MUST enabled，並與CMD-NI-012／UIIR-NI-002一致；Composer modal時背景inert，Cancel／Escape優先，不新增capability。
- L2（thread `PRRT_kwDOUFu0Cc6jPcJJ`）：F-16 source固定為selected change anchor；change absent／unavailable時固定為selected file row。Destination仍依review state為Mark Reviewed／Mark as Unreviewed；action unavailable時focus留在實際source。同步AC-NI-004／008／029，不新增shortcut、F ID或capability。
- L4（thread `PRRT_kwDOUFu0Cc6jPcJQ`）：既有AC-NI-003內互斥表達F-08 file selection的first-change normalization／no-change fallback，以及F-09 change navigation的指定target anchor＋minimum reveal／invalid-target fallback；不新增AC ID或capability。
- 不新增F／CMD／AC／UIIR／TestCase ID、product capability、shortcut、runtime API或artifact；F-17、CMD-NI-015、AC-NI-036、TC-14、UIIR-NI-007及ownership／non-BC／Human／Swift boundaries維持既有語意。

### IM-09.1 — 套用M2～M4 bounded durable corrections

- 只修改durable output；不得修改planning artifacts、requirements、prototype、Swift或其他path。M1（thread `PRRT_kwDOUFu0Cc6jQMvj`）只屬workflow truth同步，不重跑PR-12～DL-13。
- M2（thread `PRRT_kwDOUFu0Cc6jQMvm`）：selected change有效時，從selected file row的第一個標準Tab固定進該selected change anchor；anchor失效唯一fallback為同一selected file row；到達anchor後，後續標準Tab才依F-16進Review Actions。同步F-08／F-16與keyboard acceptance，不新增shortcut、ID或capability。
- M3（thread `PRRT_kwDOUFu0Cc6jQMvp`）：Composer開啟時Reader背景轉`loading`仍保持modal／background inert、editor focus及working／saved drafts原值；Back disabled且不穿透，Cancel／Escape優先。關閉後依F-14，失效content targets最終回到loading row固定的nonmodal target：首次Overview tab、重開saved active tab。不新增persistence。
- M4（thread `PRRT_kwDOUFu0Cc6jQMvr`）：Inbox `content`的native selection／local navigation保持可用；CMD-NI-002只有selected row存在且Open capability／action safety known-safe時enabled。Unknown／unavailable時disabled，stale activation no-op且selection／focus不變；同步UIIR-NI-006與applicable acceptance，不定義Logic／Integration policy。
- 不新增F／CMD／AC／UIIR／TestCase ID、product capability、shortcut、runtime API或artifact；F-17、CMD-NI-015、AC-NI-036、TC-14、UIIR-NI-007及ownership／non-BC／Human／Swift boundaries維持既有語意。
...
- PC-12只修改technical spec、plan與step三個exact paths；PR-12 `approved`後IM-08只修改durable output。兩個phase的Written／Deleted皆為None，其他paths均ReadOnly。
- PC-13只修改technical spec、plan與step三個exact paths；PR-13 `approved`後IM-09只修改durable output。兩個phase的Written／Deleted皆為None，其他paths均ReadOnly。
...
- F-16的實際source固定為selected change anchor；change不存在或unavailable時固定為selected file row。Destination依review state固定為Mark Reviewed／Mark as Unreviewed；action unavailable時focus留在實際source，不新增shortcut或F ID。
- F-08選取file且selected change有效時，從selected file row的第一個標準Tab固定進該change anchor，anchor失效唯一fallback為同一file row；到達anchor後，後續標準Tab才依F-16進Review Actions，不得跳過reading anchor。
...
- `content`時native selection／local navigation保持可用；Open只有selected row存在且capability／action safety known-safe時enabled。Unknown／unavailable時disabled，stale activation no-op且selection／focus不變。
...
- 非modal Reader `error`與`offline-without-readable-content`的Back MUST enabled並依CMD-NI-012／UIIR-NI-002返回Inbox；Composer modal時背景inert且Cancel／Escape優先，不穿透執行Back。
...
- Composer開啟時背景轉`loading`仍保持modal／inert、editor focus與working／saved drafts；Back disabled且不穿透，Cancel／Escape優先。關閉後依F-14，失效targets最終回首次Overview tab或重開saved active tab的loading-row nonmodal target。
...
- CMD-NI-002只有selected row存在且Open capability／action safety known-safe時enabled；unknown／unavailable時disabled，stale activation為no-op且selection／focus不變。Inbox `content`的native selection／local navigation不受Open disabled影響。
...
- CMD-NI-012在非modal Reader error／offline-without-readable-content維持enabled；Composer modal時背景command不執行，Cancel／Escape優先關閉Composer。
- Reader背景為`loading`且Composer modal時，Back與其他background commands disabled且不得穿透；Cancel／Escape依F-14優先關閉並恢復loading-row fixed target。
...
- Inbox PR list維持native single-selection與local navigation；Open enablement另受selected row existence與UIIR-NI-006 known-safe gate約束，disabled Open不得阻斷selection或改變focus。
...
- AC-NI-003內互斥驗收F-08與F-09：F-08 file selection依有／無可讀change分別執行first-change normalization或selected file row fallback；F-09 change navigation移到指定target change anchor並採minimum necessary scroll，invalid target保持原selection與原selected change anchor。
...
- UIIR-NI-002維持Back中性返回語意，並支援非modal Reader error／offline-without-readable-content時Back enabled；Composer modal仍由Cancel／Escape優先處理。
- UIIR-NI-006的Open capability／action safety缺失或unavailable時，CMD-NI-002採disabled／stale no-op並保持selection／focus；只定義Presentation degradation，不定義Logic／Integration policy。Composer loading規則亦不得新增persistence。

### TC-12 — SwiftUI acceptance criteria

- 每列使用穩定`AC-NI-###`、Given/context、Action、Observable result、Source trace。
- 覆蓋native routing、menus、toolbar、sidebar、resize、large diff、VoiceOver、system colors與mock-only controls；既有rows持續驗收T2無change file、T3 safe recovery、T4 missing availability、T5 Composer offline及T6 blank／stale Save Draft，AC-NI-009／010／031與AC-NI-036既有語意保持。
- 覆蓋native routing、menus、toolbar、sidebar、resize、large diff、VoiceOver、system colors與mock-only controls；applicable rows亦驗收L1非modal Back／Composer優先序、L2 F-16實際source與L4 F-08／F-09互斥行為。AC-NI-003、AC-NI-004／008／029須同步，AC-NI-036保持既有語意。
- Applicable rows驗收M2的F-08 row→anchor→F-16順序、M3 Composer／loading modal preservation與fixed close target，以及M4 Inbox Open safety gate。同步既有AC-NI-001／003／006／008／009／010／011／013／029，不新增AC ID；AC-NI-036保持既有語意。
...
- L1～L4 cycle已完成PR-12 → IM-08 → TE-10 → RV-14 → Human-confirmed DL-11 → DL-12 → DL-13；commit為`d209c61`，四threads已reply＋resolved。PC-13 entry baseline三端head一致且clean；本輪4個M1～M4 threads均unresolved／non-outdated。只有PR-13 `approved`可進IM-09，只有RV-16 `approved`可進human-confirmed commit；其後依序push，再回覆並resolve4個threads。M1不得重跑舊delivery；TC-14、ownership、non-BC與Human／Swift boundaries維持不變。
...
36. `PR-11 → DL-10` — PR-11 `approved`、IM-07 completed、TE-09 PASS、RV-12 `approved`、Human commit message confirmed、DL-08 commit `afc5616`、DL-09 push及DL-10六threads reply＋resolve均已完成；不得重跑舊delivery。
37. `RV-13` — 已明示`needs-rework`；findings只限L1非modal Back、L2 F-16 source、L3 workflow truth與L4 AC-NI-003互斥語意，4個threads仍unresolved／non-outdated。
38. `PC-12` — completed；只修改technical spec、plan與step鎖定L1～L4 semantics、acceptance、allowlists與routing，完成不構成approval。
39. `PR-12 → DL-13` — PR-12 `approved`、IM-08 completed、TE-10 PASS、RV-14 `approved`、Human commit message confirmed、DL-11 commit `d209c61`、DL-12 push及DL-13四threads reply＋resolve均已完成；不得重跑舊delivery。
40. `RV-15` — 已明示`needs-rework`；findings只限M1 workflow truth、M2 F-08→anchor→F-16 keyboard order、M3 Composer／loading modal state與M4 Inbox Open safety gate，4個threads仍unresolved／non-outdated。
41. `PC-13` — completed；只修改technical spec、plan與step鎖定M1～M4 semantics、acceptance、allowlists與routing，完成不構成approval。
42. `PR-13` — pending；只由獨立Plan-Reviewer審查PC-13，僅`approved`可進IM-09。
43. `IM-09` — pending；PR-13 `approved`後只修改durable output完成M2～M4 corrections；M1不重跑舊delivery。
44. `TE-11` — pending；IM-09後重新驗證TC-01～TC-14與M2～M4 observable results。
45. `RV-16` — pending；獨立Reviewer審查IM-09、TE-11 evidence與4 threads resolution map，僅`approved`可進delivery。
46. `DL-14` — pending；RV-16 `approved`後依`git-commit-convention`檢查staged semantic boundary，取得Human明示確認後才可commit。
47. `DL-15` — pending；DL-14 commit完成後才可push並確認local／origin head一致。
48. `DL-16` — pending；DL-15 push完成後才可依核准resolution map回覆並resolve4個M1～M4 threads。
...
- Current cycle若PR-13或RV-16為`needs-rework`，只交回對應產出角色；`blocked`或`human-check`停止交Human。未取得Human對commit boundary／message的明示確認不得commit；未完成commit不得push，未完成push不得reply或resolve4個M1～M4 threads。
...
- HC-01已由Human明示decision原值`採用`；本topic維持completed／adopted且原始adoption gate已關閉。Current M1～M4 comment cycle仍有PR-13之後的bounded correction／delivery gates，但沒有unresolved input或topic content blocker，也不重開Human adoption。
...
- T1～T6 cycle已完成PR-11 `approved`、IM-07、TE-09 PASS、RV-12 `approved`、Human-confirmed commit、DL-08 commit `afc5616`、DL-09 push與DL-10六threads reply＋resolve；PC-12 entry baseline三端head一致且working tree clean。
- L1～L4 cycle已完成PR-12 `approved`、IM-08、TE-10 PASS、RV-14 `approved`、Human-confirmed commit、DL-11 commit `d209c61`、DL-12 push與DL-13四threads reply＋resolve；PC-13 entry baseline三端head一致且working tree clean。
- 新M1～M4 cycle的RV-15實際verdict為`needs-rework`；PC-13 completed，PR-13及其後IM-09／TE-11／RV-16／DL-14～DL-16仍pending。4個threads保持unresolved／non-outdated，不得當作已核准或已交付。

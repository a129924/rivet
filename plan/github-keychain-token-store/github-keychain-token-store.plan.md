...
僅在 `feat/github-keychain-token-store` feature worktree 交付 public `KeychainTokenStore` 的 real legacy macOS Keychain implementation，並由一般 SwiftPM `@testable` tests 驗證實際 Keychain behavior。既有 GitHub token contract、opaque Security failure、single current-user identity 與 worktree policy 不變。
...
- `Package.swift`、product／target graph、existing contracts、provider、`InMemoryGitHubTokenStore`、既有非 Keychain tests、consumer package manifest、toolchain、hooks、entitlements、diagrams，以及其他 docs／BC files。
...
- shared-doc coordination 是唯一 human check：`docs/architecture/README.md` owner 未協調前，保留該 file 未寫入並如實回報；不得以未協調 writeback 宣稱完成。

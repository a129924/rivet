...
Correction／revalidation。source 與 tests 已在完整 ledger evidence 之前發生；其完成性、驗證與先前 routing 均不得由本 ledger 回填或推定。已退役的 GitHub Integration BC writeback 已由 `github-integration-boundary-redefinition` topic supersede；等待獨立 Plan-Reviewer 重審本 revision 與此受限更正，才可重建後續 implementation／verification routing。此狀態與任何 checkbox 或 artifact existence 均不構成 approval，且 Plan-Creator 不得宣告實作放行。
...
| PL-02 | Plan-Creator | completed | 依 human 對 PR review thread #3 的 option 1 決定，從四份 artifacts 退役已失效的 GitHub Integration BC writeback。 | 只移除／標記已退役 `docs/architecture/bounded-contexts/github-integration.md` 的 writeback；明記由 `github-integration-boundary-redefinition` topic 的 cross-topic artifact allowlist supersede，不指定 replacement target，且不改變 HTTP header scope、locked API、tests 或既有 gates。 | Human 選擇 option 1；四份 artifacts 同步更正。未執行產品實作、測試、commit 或品質／放行結論。 |
| PR-01 | Plan-Reviewer | pending | 由獨立角色重新審查四份 revised planning artifacts 的 scope、locked contract、已退役 writeback 與 workflow completeness。 | 明示 verdict 為 `approved`、`needs-rework`、`blocked` 或 `human-check`；不得以 artifact 或 step status 取代 verdict。僅明示 `approved` 可放行 IM-01。 | 待獨立 Plan-Reviewer 重新審查交接。 |
| CV-01 | Plan-Reviewer | pending | 獨立重審此 correction revision、已退役的 `github-integration.md` writeback 與 ledger 偏差；確認 source/tests 已先於完整 ledger evidence 發生，但不補造任何歷史 approval、完成或驗證證據。 | 明示 `approved`、`needs-rework`、`blocked` 或 `human-check` verdict；只有明示 `approved` 才可由 Dispatcher 依真實 evidence 重建後續 implementation／verification routing。 | 待獨立 Plan-Reviewer；無可回填的先前 reviewer verdict 或完整 implementation/test evidence。 |
...
- PR review thread #3 的舊 GitHub Integration writeback 已依 human option 1 標為 superseded；此處置不提供 replacement target，亦不解除其餘 correction／revalidation blockers。
...
2026-09-10（Plan-Creator 依 human option 1 處置 PR review thread #3：已退役 GitHub Integration BC writeback 由 `github-integration-boundary-redefinition` supersede；等待獨立 Plan-Reviewer）

...
- 任何 Bounded Context 文件或 architecture diagram 變更；已退役的 `docs/architecture/bounded-contexts/github-integration.md` 不得再作為 writeback target。

**ReadOnly**

- `Execution/HTTPClient.swift`、`Execution/Requester.swift`、`Execution/Transport.swift`、package manifest、所有 architecture diagrams 與所有 BC 文件：僅供理解與 regression verification，禁止寫入。
...
- 已退役 `docs/architecture/bounded-contexts/github-integration.md` 的 writeback 已由 `github-integration-boundary-redefinition` topic supersede；本 topic 不新增 replacement writeback target，所有 BC 文件與 diagram（包含 package-structure diagram）皆為 read-only。

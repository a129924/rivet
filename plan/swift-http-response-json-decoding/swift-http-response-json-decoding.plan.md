...
- 最小更新 HTTP client architecture 文件與兩張既有 diagram，記錄 caller-owned decoder convenience 及 raw response boundary；generated `index.html` 僅經 artifact-local workflow rebuild。
...
- HTTP client architecture 文件與兩張既有 diagram 的最小事實回寫，說明 caller-owned decoder convenience 不改變 raw response dataflow。
...
- `docs/architecture/README.md`、`docs/architecture/bounded-contexts/github-integration.md`：最小回寫 package 提供 caller-owned decode convenience 的長期事實。
- `docs/architecture/diagrams/github-integration-http-client-boundary/scene.js`、`docs/architecture/diagrams/http-client-package-structure/scene.js`：最小回寫 HTTP response convenience 與既有 ownership boundary。
...
- 僅當對應 `scene.js` 有變更時，依各自 artifact-local workflow 產生並寫入兩張 diagram 的 `index.html`；不得直接手改生成檔或發布 artifact.cafe。
...
- 既有 `HTTPResponse` 的 raw `body`、`statusCode`、headers contract 持續通過；執行 standalone package checks 與兩張 diagram 的 validate、build、enhance、accessibility verification。
...
- 當 diagram source 有變更，依每張 artifact 的 `BUILD.md` 執行 validate、build、enhance 與 accessibility verification；確認 generated artifact 由 workflow 產生。

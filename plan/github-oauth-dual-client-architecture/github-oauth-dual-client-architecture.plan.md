...
- `GithubIntegration` non-BC shared module、bare `RivetHTTPClient`、layer 外 application composition root、Provider lifecycle、REST／GraphQL consumer boundaries、401-only recovery／one retry／second-401 terminal 與 failure classification。
- 保留 v1／v2／v3／v4／v5 lifecycle artifacts 作 immutable rejected evidence、建立 canonical manifest、補強已驗證交付的繁體中文 v6 lifecycle 的 expiry-valid snapshot、post-rotation persistence visible/deferred outcome，以及不改語意的 canvas colour fix 與 REST → bare executor edge。
...
- **Locked content**：
  ...
  - TokenStore 儲存完整 credential bundle；Fetcher 僅以 bare HTTP refresh；TokenSnapshot 僅是 access token + version。本 topic 不取代既有 `GitHubTokenProvider`／Store boundary，也不定義 migration。
  ...
  - client 取得 expiry-valid snapshot；stale version 直接取現行 snapshot；same stale version concurrent 401 single-flight refresh；只有 401 觸發 recovery，每個原工作最多 retry 一次。
  - 無 credential、permanent refresh failure、second 401 為 terminal authentication-required；僅未接受遠端 rotation 前的 transient technical failure 保留 credential。post-rotation persistence failure 是 visible technical failure，不宣稱舊 bundle 可用並 defer reconciliation；403／權限／資源可見性不 refresh。
...
- **Action**：
  ...
  - 保留 v1／v2／v3／v4／v5 lifecycle artifacts 作 immutable rejected evidence，在 diagrams allowlist 新增 canonical manifest，將已驗證交付的繁體中文 v6 lifecycle 指為 canonical artifact。
  - 使用 `architecture-canvas` 建立繁體中文責任／依賴 canvas，不表達 runtime sequence；本輪 canvas 僅作 colour fix 與 GitHub REST client → bare HTTP sender／request executor edge，不改其他 diagram semantics。
  - 使用 `archify` 建立繁體中文 v6 lifecycle：expiry-valid snapshot → Bearer → 401-only provider recovery → one retry／second-401 terminal outcome，並補上 post-rotation persistence visible/deferred reconciliation boundary。
...
- **TC-04**：artifact contract 維持 Provider／client ownership、expiry-valid snapshot、single-flight、401-only recovery、one retry、second-401 terminal、permanent／transient distinction、post-rotation persistence visible/deferred reconciliation 與 403 no-refresh。
- **TC-05**：independent Plan Review 未 `approved` 前，沒有 `docs/architecture/` 或 diagrams 寫入。
- **TC-06**：Phase 2 保留 v1／v2／v3／v4／v5 作 immutable rejected evidence、加入 canonical manifest 並指認已驗證交付的繁體中文 v6 canonical lifecycle；v6 包含 expiry-valid snapshot、second-401 terminal、post-rotation persistence visible/deferred outcome，canvas colour fix 與 REST → bare executor edge 不改其他語意。

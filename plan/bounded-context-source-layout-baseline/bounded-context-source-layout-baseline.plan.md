...
- 未經新的獨立 REVIEW-007 明示 `approved` 的 audit-correction commit、push、PR thread resolution、release，或進入下一個產品 slice。
- 本次 PR review 已明示授權的三份既有 BC 文件回寫為既有 remediation 例外；不得據此修改其他架構文件或改變既定三 BC baseline。
- `94b6506` 早於 REVIEW-001，且 REVIEW-001 的歷史 verdict 為 `needs-rework`；`09043bf`、`ed2bbcf`、`768f6be`、`1b44fd0` 與 `a1e5b4a` 分別早於未執行的 REVIEW-002、REVIEW-003、REVIEW-004、REVIEW-005 與 REVIEW-006。不得倒填、改寫、force push 或宣稱任一 commit 曾獲相應 review 核可。後續 DELIVERY-005 必須在新的獨立 REVIEW-007 明示 `approved` 後才可執行。

## ReadOnly

Layout Implementation 僅可讀取 `AGENTS.md`、`README.md`、設計原則、架構、本 topic planning artifacts、`scripts/check-swift-format.sh`、既有 renderer check 與必要時的 `.gitignore`。`requirements.md`、`technical-spec.md` 與本 plan 在 Layout Implementation 開始後一律為 ReadOnly；五次 Human 明示授權的 planning-artifact amendment 是唯一例外，且第五次僅可記錄 `a1e5b4a` 的 REVIEW-006 historical deviation 與後續 flow。
...
Plan-Creator 只可建立或更新本 topic 四份 artifacts。Human 明示授權的 PR review planning-artifact amendment 僅可記錄既已授權的 BC-document remediation、historical audit 與 delivery consistency；第二次 amendment 僅可如實記錄三個 commit 越過的 review gate、加入獨立 `verdict` 欄並建立 REVIEW-004 → DELIVERY-002 → HUMAN-002；第三次 amendment 僅可如實記錄 `768f6be` 早於未執行的 REVIEW-004，並建立 REVIEW-005 → DELIVERY-003 → HUMAN-003；第四次 amendment 僅可如實記錄 `1b44fd0` 早於未執行的 REVIEW-005，並建立 PLAN-011 → PLAN-012 → TEST-007 → REVIEW-006 → DELIVERY-004 → HUMAN-004；第五次 amendment 僅可如實記錄 `a1e5b4a` 早於未執行的 REVIEW-006，並建立 PLAN-013 → PLAN-014 → TEST-008 → REVIEW-007 → DELIVERY-005 → HUMAN-005；不得重新定義三 BC baseline、paths 或 contracts。Implementer 只可 create-only 接觸：
...
允許修改的既有 tracked paths 僅為 `scripts/check-swift-format.sh` 與三份明列 BC 文件。formatter 僅可將既有目錄 lint 調整為使用 `--recursive`；BC 文件僅可加入上述單行 baseline；不得變更其他 formatter 行為、設定、path、BC responsibility、Map 或圖。五次 Human 明示授權的 planning-artifact amendment 是上述限制的唯一 artifacts 例外；第五次僅可記錄 `a1e5b4a` historical deviation 與 PLAN-013 → PLAN-014 → TEST-008 → REVIEW-007 → DELIVERY-005 → HUMAN-005，不得改變 scope 或 contract。

## TestCase

- 四份 planning artifacts 在 Layout Implementation 前已建立；實作 phase 不改動 `requirements.md`、`technical-spec.md` 或本 plan。五次 Human 明示授權的 planning-artifact amendment 是唯一例外；第五次僅可如實記錄 `a1e5b4a` 早於 REVIEW-006 與 PLAN-013 → PLAN-014 → TEST-008 → REVIEW-007 → DELIVERY-005 → HUMAN-005；`.step.md` 僅可依明示 human 授權更新。
...
- 如實記錄 `94b6506` 早於 REVIEW-001，且 REVIEW-001 的歷史 verdict 為 `needs-rework`；`09043bf`、`ed2bbcf`、`768f6be`、`1b44fd0` 與 `a1e5b4a` 分別早於未執行的 REVIEW-002、REVIEW-003、REVIEW-004、REVIEW-005 與 REVIEW-006。不得倒填、宣稱 prior approval、rewrite history 或 force push。C thread 的 PR Inbox-only 要求是與 Human 鎖定三 BC decision 衝突的 non-actionable finding。
...
- 第五次 planning-audit amendment 完成後，PLAN-014 必須明示 `approved`，TEST-008 必須獨立驗證 artifacts audit consistency；新的獨立 REVIEW-007 明示 `approved` 前，不得進行 DELIVERY-005 commit、push 或 PR thread resolution。
- DELIVERY-005 僅可包含本次 planning-audit amendment 的四份 topic artifacts；不得變更既有 commits、BC 文件、source layout 或其他 tracked path。
...
- 第五次 planning-audit amendment 未經 PLAN-014 明示 `approved`，或 TEST-008 未完成驗證、REVIEW-007 未明示 `approved`。

## Human Check Gate

既有 PR review remediation 與 audit-correction delivery 均為歷史；僅可在第五次 planning-audit amendment 經獨立 PLAN-014 與 REVIEW-007 明示 `approved`，並由 TEST-008 完成驗證後，建立 DELIVERY-005 commit、push branch 並處理已核可的 PR threads。不得重做、改寫或 force push `94b6506`／`09043bf`／`ed2bbcf`／`768f6be`／`1b44fd0`／`a1e5b4a`；完成 DELIVERY-005 後停止於 HUMAN-005 human review，不得 merge、release 或進入下一個產品 slice。

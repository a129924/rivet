...
| AC-NI-010 | Reader依序收到五種presentation state | 觸發loading、content、error、兩種offline；在Review Action持有focus時轉入offline-readable | 每態九欄行為成立；readable content保留selection／progress；content row的明示transition targets全失效時terminal focus為目前active Reader tab；disabled Review Action依current change anchor → selected file row恢復focus，Files content失效才進state heading；無content時不顯示diff或可用review action | §5；Draft 3.2；TC-05 |
...
| Reader Presentation State Matrix | Draft 3.2、7B | Adopted Reader workspace、per-PR selection／progress continuity與active-tab terminal focus | TC-05 |
| Command Matrix | Draft 3.3、7C | Baseline Files-only shortcuts、Composer guard、selected-row-only context Open、Mark／Unreview、Finish／Back behavior | TC-06 |

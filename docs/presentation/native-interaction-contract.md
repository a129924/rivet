...
| F-14 | Comment Composer | Cancel、Escape或合格的Save Draft關閉Composer；disabled／stale Save不形成F-14 transition | 固定ordered chain：opening target → current selected change anchor → selected file row；只有selected file亦失效時，才交給current Reader Presentation State Matrix明示的focus target。若Composer開啟期間background已成為offline-without-readable-content，Cancel／Escape關閉後focus **MUST** 落在目前offline-state heading | Cancel丟棄working copy但保留既有saved draft；成功Save只改draft；selection／review state不變 | Keyboard `C`的opening target就是觸發前focus target；**MUST NOT** 跳過chain中仍有效的target；offline-without-readable-content時referenced content targets視為失效 | Cancel或成功Save **MUST** 宣告實際結果；disabled／stale Save **MUST NOT** 宣告draft saved；不得重讀整個diff |
...
| `error` | 可保留的 Reader chrome **MUST** 保留 | Error status；若 current Reader content仍 readable，**MUST** 保留並清楚標示非最新；availability missing時 **MUST NOT** 顯示Reader content或file rows | 只有 referenced content仍 readable時，才保留其 selection、review progress與scroll anchor；availability missing時只保存可恢復identity | Availability missing時為error heading；否則有效current target保持，無有效target才落error heading | Known-safe Retry／recovery與readable content的本地閱讀／導覽 **MAY** enabled | Safety unknown的recovery、需要unavailable capability，以及沒有內容的review／comment／Finish **MUST** disabled | Recovery affordance **MUST** 清楚呈現；unknown時以可理解disabled semantics保留 | 一次性宣告failure、內容是否仍可讀及recovery是否disabled；不得清空進度後再宣告零進度 |
...
| CMD-NI-002 | Enter：Open Reader | Selected Inbox PR row | Row存在且可開啟 | Context不成立時保持 state與focus；不得切 tab或 workspace | 依F-02開啟selected PR：首次進Overview target；重開則恢復saved active tab及其deterministic content target | `Open Selected Pull Request` 等價 menu command **MUST** 存在 | 只有clicked PR row同時為selected row時 **MAY** 提供等價 `Open`；clicked non-selected row **MUST NOT** 顯示或執行 `Open`，亦不得為此先改selection |
...
Missing-input degradation: Availability missing的唯一safe state是error；不得推測offline或顯示Reader content／file rows。Focus落error heading，保留Back與可理解recovery affordance；recovery依UIIR-NI-006僅在known-safe時可用，unknown時disabled。
...
Missing-input degradation: Action保持disabled並提供可理解的disabled semantics；不得樂觀執行、偷偷切tab或宣告成功。Stale Save Draft activation必須no-op，並保持Composer、editor focus、working copy與saved draft原值，且不宣告draft saved。
...
| AC-NI-003 | Files有多個files，且target file分別有／無可讀change | 選file或navigate change | File selection留在row；有可讀change時設第一個change並以anchor minimum reveal，無可讀change時selected change absent／unavailable、focus file row、不造假anchor且change-dependent navigation／comment disabled | F-08／F-09；Baseline Interaction Contract；TC-03／TC-09 |
| AC-NI-004 | Current file未reviewed且next unreviewed file分別有／無可讀change | 啟用Mark Reviewed | 選next unreviewed；有可讀change時選第一個並focus anchor，無可讀change時selected change absent／unavailable、focus new file row且change-dependent actions disabled；無pointer可繼續可用流程 | F-10；Draft 3.1；Baseline Review Progress；TC-03 |
...
| AC-NI-006 | Files reading target有效；另含Composer開啟後background轉offline-without-readable-content，以及blank／stale Save | 開啟Composer、轉換background state，再Cancel／Escape或嘗試Save | Transition後Composer與editor focus、working copy、saved draft保持且background inert；Cancel／Escape關閉後focus offline heading。Save只在trimmed非空且content/action known-safe時成功；blank disabled、stale no-op，皆不改Composer/drafts或宣告saved | F-13／F-14；CMD-NI-008；§5；Baseline TC-12；TC-03／TC-05／TC-06 |
...
| AC-NI-009 | Inbox依序收到六種presentation state，並分別提供missing／known-offline availability與known／unknown recovery safety | 觸發loading、content、empty、error、兩種offline | 每態九欄行為成立；missing availability只進error、無rows／Open且focus error heading；offline只在known offline使用；recovery只有known-safe可用，unknown保留可理解disabled affordance | §4；UIIR-NI-001／006；Draft 3.2；TC-04 |
| AC-NI-010 | Reader依序收到五種presentation state，並分別提供missing／known-offline availability與known／unknown recovery safety | 觸發loading、content、error、兩種offline；在Review Action或Composer持有focus時轉state | 每態九欄行為成立；missing availability只進error且focus error heading；offline只在known offline使用；recovery僅known-safe可用。Readable content保留selection／progress；content targets全失效時terminal focus為active tab；offline-without時Composer保持modal/editor/drafts，Cancel／Escape後focus offline heading | §5；UIIR-NI-002／006；Draft 3.2；TC-05 |
...
| Focus State Machine F-01～F-17 | Draft 3.1、7A | Baseline technical spec的Workspace State Machine／Interaction Contract；首次開啟workflow、no-readable-change fallback與Composer offline close focus | TC-03 |
| Inbox Presentation State Matrix | Draft 3.2、7B | Adopted Inbox workspace、selection／return continuity、missing-availability error與known-safe recovery | TC-04 |
| Reader Presentation State Matrix | Draft 3.2、7B | Adopted Reader workspace、per-PR continuity、active-tab terminal focus、missing-availability error與Composer modal preservation | TC-05 |
| Command Matrix | Draft 3.3、7C | Baseline Files-only shortcuts、no-change guards、selected-row-only context Open、Composer blank／stale Save guard、Mark／Unreview、Finish／Back behavior | TC-06 |
...
| UI Input Requirements boundary | Draft 4、5 | Repository layer／ownership invariants；missing availability、known-safe recovery與baseline prototype-local Presentation state boundary | TC-11 |
| SwiftUI observable acceptance criteria | Draft 3.8、7F、9 | 全部adopted workflow evidence、no-change／recovery／Composer edge cases與HC-03 adoption | TC-12 |

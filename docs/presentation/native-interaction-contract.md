...
| F-16 | Current selected change anchor；selected change absent／unavailable時為current selected file row | 標準 Tab 向前進入 Review Actions | Current未reviewed時為 `Mark Reviewed`；current已reviewed時為 `Mark as Unreviewed`；對應action不可用時，focus **MUST** 留在實際source（selected change anchor或selected file row） | Selection與progress不變 | 這是標準focus traversal，不建立shortcut；selected change absent／unavailable時 **MUST NOT** 造假anchor | Action的名稱、enabled state與目前file **MUST** 可讀；留在file row時須保留no-readable-change語意 |
...
Presentation use: 決定Reader matrix row、content是否保留、哪些本地reading/navigation actions可用、nonmodal Back及recovery affordance。
...
| AC-NI-008 | Review Actions可用，且selected change分別present與absent／unavailable | 從actual reading source標準Tab進入Review Actions，再Shift-Tab返回 | Present時source為selected change anchor，absent／unavailable時source為selected file row；依review state進Mark／Unreview，action unavailable時留actual source；Shift-Tab依F-17回change anchor或file row fallback | F-16／F-17；Draft 3.1；TC-03 |
...
| AC-NI-029 | 不使用pointer，且Files分別有／無selected change | 完成Inbox→Reader→Files→Mark／Unreview→Comment→Finish／Back；Comment只在change present且safe的branch執行 | 全流程可由keyboard與menu完成；有change從anchor、無change從file row進依review state決定的action，unavailable時留actual source；no-change branch不執行disabled Comment；nonmodal error／offline-without仍可Back，Composer modal時Back不穿透；focus ring與restoration持續可見／可讀 | §3／§5／§6／§10；Draft 3.1／3.7；TC-03／TC-05／TC-06／TC-10 |
...
| Focus State Machine F-01～F-17 | Draft 3.1、7A | Baseline technical spec的Workspace State Machine／Interaction Contract；首次開啟workflow、actual Review Action source、no-readable-change fallback與Composer offline close focus | TC-03 |
...
| Reader Presentation State Matrix | Draft 3.2、7B | Adopted Reader workspace、per-PR continuity、nonmodal Back、active-tab terminal focus、missing-availability error與Composer modal preservation | TC-05 |
| Command Matrix | Draft 3.3、7C | Baseline Files-only shortcuts、no-change guards、selected-row-only context Open、Composer blank／stale Save guard、Mark／Unreview、Finish與modal-aware Back behavior | TC-06 |

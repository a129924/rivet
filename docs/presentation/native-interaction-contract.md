...
- 主流程 **MUST（必須）**保持：Inbox → Open PR → Reader Overview → Review Files → Files／Diff → Mark Reviewed → Next File → Continue Review → All Files Reviewed → Finish Reading → Inbox。
...
下表中的 fallback 只在 primary destination 於 event 完成時不存在或不可接收 focus 時使用。若整個 referenced content 已失效，對應 Presentation State Matrix **MUST（必須）**接管 focus；implementation **MUST NOT（不得）**再猜測第三個 destination。
...
| F-02 | Inbox selected PR row | Enter、double-click 或 Open Reader | Reader Overview 的 PR title heading；heading 尚未建立或不可 programmatically focused 時，唯一 fallback 為 enabled Overview tab | Active PR 設為原 selected PR；不得改 reviewed state | 保存來源 Inbox selection row | Workspace 變更 **MUST** 以 title／Overview context 表達，避免重複朗讀整個 Reader |
...
| F-04 | Active Overview tab | 標準 Tab 向前離開 tab strip | Overview PR title heading；heading 失效時，唯一 fallback 為 Overview tab | PR、tab 與 review state不變 | 每次進 Overview 都從 title heading 開始，不恢復任意舊 content focus | Heading **MUST** 具有可理解的 heading semantics |
...
| F-08 | File Navigator 的 selected file row | Click 或 native list selection 選另一 file | 新 selected file row；target file 在 event 完成前失效時，唯一 fallback 為原 selected file row | 選 target file，change 正規化為該 file 第一個 change；失效時 selection 不變 | 不恢復 target file 的舊任意 change；reviewed membership 不變 | Row **MUST** 同時表達 current 與 reviewed／unreviewed；更新採 local announcement |
| F-09 | File Navigator 或 Diff／Change Reader | Arrow Up／Down、change control 或 click 選另一 change | 新 selected change anchor；target change 失效時，唯一 fallback 為原 selected change anchor | 只在 current file 內更新 selected change | Minimum necessary reveal；不得無條件置中 | Announcement **SHOULD** 包含 change position與hunk摘要，並去重複 |
...
| F-14 | Comment Composer | Cancel、Escape 或 Save Draft 關閉 Composer | Opening target；opening target 已失效時，唯一 fallback 為 current selected change anchor | Cancel 丟棄 working copy但保留既有 saved draft；Save 只改 draft；selection／review state不變 | Keyboard `C` 的 opening target就是觸發前 focus target | **MUST** 宣告取消或 draft saved；不得重讀整個 diff |
...
| `offline-with-readable-existing-content` | Toolbar、tabs與 active content chrome **MUST** 保留，並顯示 offline／stale indicator | Existing readable Overview／Files／Commits／Checks content | PR、tab、selection、review progress、draft與scroll anchor **MUST** 保留，只要其 referenced content仍有效 | 目前有效 focus target；失效時使用該 region於 F-table 的唯一 fallback | 本地 reading、selection、change／file navigation與 Back **MUST** enabled；其他 actions依 UI Input Requirement 的 safe availability決定 | 需要 network或無法安全執行的 actions **MUST** disabled | 提供 reconnect／retry；不得阻擋本地閱讀 | 一次性宣告 offline、內容仍可讀及 disabled能力；後續 navigation保持局部 |
...
| CMD-NI-002 | Enter：Open Reader | Selected Inbox PR row | Row存在且可開啟 | Context不成立時保持 state與focus；不得切 tab或 workspace | 開啟 selected PR，依 F-02 focus Reader Overview | `Open Selected Pull Request` 等價 menu command **MUST** 存在 | Selected／clicked PR row **MAY** 提供等價 `Open` |
...
| CMD-NI-004 | Review Changes／Continue Review | Overview action | Review count分別為零／部分且 Files content可用 | Context或content不可用時 no-op並保持focus | 啟用 Files，保留 selected file／change與review state，依 F-05進入 | Reader menu **MUST** 提供語意等價 action | None |
| CMD-NI-005 | Arrow Up／Down：Change navigation | Files，Diff／Change Reader reading context | Current file有 previous／next change，且無 modal／editable／VoiceOver conflict | 首尾 no-op；非 Files不得切到 Files；conflict時不攔截 | 只更新 current file內 selected change並依 F-09 reveal／focus | Reader menu **SHOULD** 提供 Previous／Next Change | Current diff／change context menu **MAY** mirror existing navigation |
| CMD-NI-006 | Option+Arrow Up／Down：File navigation | Files reading context | 有 previous／next file，且無 modal／editable／VoiceOver conflict | 首尾 no-op；非 Files不得切 tab；conflict時不攔截 | 選 previous／next file，change設為第一個，reviewed state不變 | Reader menu **MUST** 提供 Previous／Next File | File row／diff context menu **MAY** mirror existing file navigation |
...
| CMD-NI-009 | Mark Reviewed | Review Actions；current file未reviewed | Files content可用且 event仍合格 | Stale／ineligible／offline-unsafe時 no-op；保持selection與focus或由state matrix接管 | 未完成依 F-10前進；完成依 F-11留位；只改既有review presentation state | Reader menu **MUST** 提供 `Mark Reviewed` 並反映 enablement | Current／clicked file row **MAY** mirror同一能力；作用於明示item |
| CMD-NI-010 | Mark as Unreviewed | Review Actions；current file已reviewed | Files content可用且 event仍合格 | Stale／ineligible／offline-unsafe時 no-op；不得誤用 Mark forward algorithm | 只移除 current reviewed state並依 F-12恢復 focus | Reader menu **MUST** 提供 `Mark as Unreviewed` | Current／clicked reviewed file row **MAY** mirror同一能力 |
...
| File navigator | Native single-selection list | Current selection與reviewed state **MUST** 同時可表達；list **MUST** 支援full keyboard access；collapse不得遺失selection | Previous／Next File、Mark／Unreview與適用context menu依Command Matrix | Filename、current、reviewed、stats與comment count（若有）**MUST** 為獨立semantics | **MUST NOT** 將row indicator變成未授權toggle或寫入GitHub viewed state |
...
| Context menus | Native contextual menu | 作用target **MUST** 是clicked item；沒有clicked item時才使用明示selected item | 只mirror該context已存在且可用的Open、Comment、Mark／Unreview等能力 | Enablement **MUST** 與menu／toolbar一致 | **MUST NOT** 新增product capability、偷偷切tab或改selection後再執行 |
...
- **Change navigation**：Arrow Up／Down或等價command **MUST（必須）**只在current file內移動selected change；首尾 **MUST（必須）**no-op。新anchor以minimum necessary scroll reveal，並依F-09接收focus。
- **File navigation**：Option+Arrow Up／Down或等價command **MUST（必須）**只移動file selection；首尾 **MUST（必須）**no-op。新file選取後 **MUST（必須）**選第一個appropriate change並reveal；reviewed state不得因導覽改變。
...
- **Overview → Files restoration**：返回Files **MUST（必須）**恢復該PR的selected file、selected change與合理scroll anchor。Saved anchor失效時，唯一fallback為selected file的第一個appropriate change；不得選擇任意可見hunk。
...
Missing-input degradation: 使用selected file的第一個appropriate change；若file也失效，交由Reader presentation state處理，不猜測任意可見hunk。
...
| --- | --- | --- | --- | --- |
| AC-NI-001 | Inbox有可選PR且app初次進入 | 啟動app或進入Inbox | Focus在恢復row，否則第一row；Enter開Reader後focus到Overview title，fallback只到Overview tab | F-01／F-02；Draft 3.1；Baseline TC-05；本topic TC-03 |
| AC-NI-002 | Reader tab strip可用 | Click、Enter／Space、Arrow Left／Right、Home／End啟用各tab，再Tab進content | Focus先留active tab；Overview／Files／Commits／Checks分別進入F-04～F-07固定target與fallback | F-03～F-07；Draft 3.1；本topic TC-03 |
...
| AC-NI-006 | Files reading target有效 | 開啟、Cancel、Escape或Save Composer | Focus進editor且modal containment；關閉回opening target，失效只回current change；selection／progress不變 | F-13／F-14；Baseline TC-12；TC-03 |
...
| AC-NI-010 | Reader依序收到五種presentation state | 觸發loading、content、error、兩種offline | 每態九欄行為成立；readable content保留selection／progress，無content時不顯示diff或可用review action | §5；Draft 3.2；TC-05 |
...
| AC-NI-013 | Toolbar、menu與適用context menu同時可見 | 比較既有action的名稱、enablement與執行結果 | Toolbar action有等價menu；context menu只mirrorclicked／selected item既有能力，三處結果一致 | §6／§7；Draft 3.3／3.4；TC-06／TC-07 |
...
| AC-NI-020 | Large diff含多file／change | 使用Arrow與Option+Arrow | Change只在file內移動；file移動選第一appropriate change；首尾no-op；reviewed state不變 | §9；Draft 3.6；Baseline navigation；TC-09 |
...
| AC-NI-023 | Files有saved selected file／change／scroll anchor | 從Overview返回Files或reveal target | 恢復三者並只做minimum necessary scroll；anchor失效只fallback到同file第一appropriate change | §9；F-05；Draft 3.6；TC-09 |
...
| Focus State Machine F-01～F-17 | Draft 3.1、7A | Baseline technical spec的Workspace State Machine／Interaction Contract | TC-03 |

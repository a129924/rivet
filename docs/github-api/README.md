...
- GitHub REST／GraphQL 的共通 API version、共通 request header 與 authentication mechanism 屬 `GithubIntegration` 的未來實作決策；endpoint-specific media type 仍由各 BC Infra 決定。文件只保留 endpoint 特有的 media type 與限制。

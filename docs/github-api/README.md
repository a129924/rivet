...
本目錄是 Rivet 各 Domain BC 未來進行 GitHub Infra REST／GraphQL 外部協定工作時的 API 參考。`GithubIntegration` 是 BC 外、尚未實作的 shared GitHub-specific integration module，不是 Supporting BC；它未來只集中可重用的 GitHub raw transport、authentication、共通 request headers／API version mechanism、pagination、rate-limit handling、retry／transient-failure handling、GitHub error technical classification 與 shared configuration。這些只是在 boundary 上可共享的未來 mechanism responsibility，不宣稱任何 runtime policy 已實作。各 BC Infra 則各自擁有 endpoint-specific media type、REST／GraphQL external-to-BC translation、將該 technical classification 映射為 BC failure contract，以及 business meaning。本目錄不定義 Swift contract、BC data model 或 authentication 實作；那些決策必須留給對應的 implementation topic。
...
| PR 基本資訊、REST changed files、unified diff | MVP 唯讀 | [REST PR Reader](rest-pr-reader.md) |
| Checks | MVP 唯讀 | [GraphQL PR State](graphql-pr-state.md) |
| PR conversation、inline comments、reviews | MVP 唯讀 | [REST Discussion & Reviews](rest-discussion-reviews.md) |
| GraphQL Files Viewed state 讀取與變更後失效 | 後續唯讀 | [GraphQL PR State](graphql-pr-state.md) |
...
- GitHub REST／GraphQL 的共通 API version、共通 request header、authentication、pagination、rate-limit handling 與 retry／transient-failure handling，都是 `GithubIntegration` 未來可共享的 mechanism responsibility，不代表現有 runtime policy；endpoint-specific media type 仍由各 BC Infra 決定。文件只保留 endpoint 特有的 media type 與限制。

...
本目錄是 Rivet 未來 GitHub Integration 進行 GitHub REST／GraphQL 外部協定工作時的 API 參考。GitHub Integration 是 Supporting BC，隔離外部協定與資料；本目錄不定義 Swift contract、BC data model 或 authentication 實作；那些決策必須留給對應的 implementation topic。
...
- GitHub REST／GraphQL 的 API version、Accept header 與 token 細節屬 GitHub Integration 的未來實作決策；文件只保留 endpoint 特有的 media type 與限制。

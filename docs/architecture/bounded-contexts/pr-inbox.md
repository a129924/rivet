...
- 未來的 non-BC `GithubIntegration` 若被建立，只有此 Infra 可依賴它；它集中 GitHub-specific technical mechanisms，但不依賴任何 BC。GitHub DTO translation、failure mapping 與 Inbox business meaning 仍留在此 Infra；它們不會進入 Core、UseCase 或 Port。

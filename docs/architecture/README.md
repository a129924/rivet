...
- **RivetHTTPClient**：BC 外的 GitHub-unaware generic technical foundation；在 caller 明確提供 `Auth` 的 `HTTPClient.execute(_:auth:)` overload 中，HTTPClient 建立並驅動 `AuthFlow` 的 `.send → raw HTTPResponse → receive` loop，但 flow 保有認證決策。
...
- [HTTP Client Auth Flow 合約生命週期](diagrams/http-client-auth-flow-contract/auth-flow-lifecycle.html)：caller opt-in 的 `HTTPClient.execute(_:auth:)` 建立並驅動 generic `Auth`／`AuthFlow` state-machine：`.send(HTTPRequest) → raw HTTPResponse → receive(response)`，直到 `.finish`；flow 保有認證決策，runtime 不包含 credential、retry 或 GitHub-specific behavior。
- [HTTP Client package 結構](diagrams/http-client-package-structure/index.html)：`HTTPClient` 提供 `request(...)`、常見 HTTP method facade 與 opt-in auth execute overload，經 `Requester → Transport` 形成 raw `HTTPURL → HTTPRequest → HTTPResponse` 鏈；包含 `URLSessionTransport`、caller-owned JSON convenience 與 header convenience，但不包含 Endpoint 組裝。

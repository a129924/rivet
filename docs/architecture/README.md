...
本文件記錄未來實作必須遵守的架構不變量。Rivet 目前是 architecture baseline；除已鎖定的 PR Reader WebView diff declaration-only modules、其單一架構圖的 artifact-local viewer 可近用性補強，以及 `RivetHTTPClient` 的最小 HTTP 介面切片外，不建立產品功能。

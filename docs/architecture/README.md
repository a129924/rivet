...
- **RivetHTTPClient**：BC 外的 GitHub-unaware generic technical foundation；`HTTPClient` 只執行 bare HTTP request。internal `AuthRequester` 以 injected `Requester` 與 caller-provided `Auth` 建立並驅動 `AuthFlow` 的 `.send → raw HTTPResponse → receive` loop，但 flow 保有認證決策。
...
- [HTTP Client package 結構](diagrams/http-client-package-structure/index.html)：`HTTPClient` 提供 bare `request(...)` 與常見 HTTP method facade，經 `Requester → Transport` 形成 raw `HTTPURL → HTTPRequest → HTTPResponse` 鏈；internal `AuthRequester` 是 `Requester` 與 `Auth` 的 generic decorator。圖包含 `URLSessionTransport`、caller-owned JSON convenience 與 header convenience，但不包含 Endpoint 組裝。

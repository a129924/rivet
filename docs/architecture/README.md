...
- **RivetHTTPClient**：BC 外的 generic technical foundation，為 GitHub-unaware internal package，擁有 declarations-only 的 `ClientAction`、`Auth` 與 `AuthFlow` contract；`HTTPClient` 不建立、驅動或解讀 AuthFlow。

...
- `AuthRequester` 是 caller original request 的唯一 owner，也是 semantic decision interpreter；不得重新制定 retry policy。
...
Model C 的 target 只允許 flow 產出 semantic decision。`AuthRequester` 保留 caller intent 並解讀該 decision，必要的 generic HTTP I/O 仍委派給 `Requester`。exact factory signature、semantic action case、request decoration representation、failure surface、async boundary，以及 refresh result delivery 都尚未鎖定。
...
| `AuthRequester` | 否 | 是，轉交給 flow | 是，唯一 owner | 是，僅保留 caller intent；representation deferred | 否 | 否，委派給 `Requester` | 否 |
...
在 normal request 中，caller 把 original request 交給 `AuthRequester`。後者取得 independent flow、解讀 flow 的 semantic send decision，並讓 `Requester` 執行保留的 caller intent。raw response 經 `AuthRequester` 交給 `AuthFlow`；flow 做 terminal policy decision，然後由 `AuthRequester` 回傳 response。

在 first 401 中，唯一的 refresh/retry policy owner 是 `AuthFlow`。`AuthRequester` 只解讀 semantic refresh decision，將它交給 **deferred refresh I/O boundary**。該 boundary 的名稱、endpoint、payload、credential persistence 和 result API 都未定義。refresh success 後，flow 可授權一次 preserved-original retry；second 401 或 refresh failure 都必須由 flow 導向終態。這是 target topology，不是既有 runtime 行為。
...
後續 Swift implementation 必須先以獨立 topic 鎖定上述 deferred interface，並驗證：normal response terminal、first 401 requests refresh、refresh success permits exactly one preserved-original retry、second 401 terminal，以及 refresh failure terminal。它也必須以 type/API evidence 證明 `AuthFlow` 無 arbitrary endpoint/request construction 或 I/O capability。除非另開 topic 重新檢討，本 decision 不處理 generic retry、5xx/network retry、backoff、rate limit、single-flight refresh、OAuth lifecycle 或 credential storage。

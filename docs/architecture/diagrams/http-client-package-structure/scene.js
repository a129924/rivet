...
const BOXES = [
  ...
  { id:'http-client', plane:'client', band:'band-client', x:140,y:620,w:545,h:68,r:10,dash:true,
    name:'HTTPClient',about:'公開 facade 與 execute 入口；auth overload generic 地驅動 AuthFlow，其餘 facade 經 execute 委派給 Requester。',
    texts:[['bl',164,648,'HTTPClient'],['bs',164,670,'facade → execute(_:)；auth overload 驅動 flow']] },
  { id:'requester', plane:'client', band:'band-client', x:727,y:620,w:592,h:68,r:10,dash:true,
    name:'Requester',about:'把已驗證 HTTPRequest 映射為 Foundation URLRequest；不重驗 URL。',
    texts:[['bl',751,648,'Requester'],['bs',751,670,'HTTPRequest → URLRequest']] },
...
const EDGES = [
  ...
  { from:'http-client',to:'requester',pts:[[690,654],[719,654]],label:{s:'al',x:705,y:642,t:'依賴',anchor:'center'} },
  { from:'http-client',to:'auth-flow',pts:[[620,614],[620,555],[554,555],[554,543]],label:{s:'al',x:637,y:580,t:'驅動',rot:-90,anchor:'center'} },
  { from:'http-client',to:'request',pts:[[420,614],[420,555],[837,555],[837,479]],label:{s:'al',x:854,y:568,t:'使用',rot:-90,anchor:'center'} },
  { from:'http-client',to:'response',pts:[[465,614],[465,557],[1154,557],[1154,479]],label:{s:'al',x:1171,y:568,t:'回傳型別',rot:-90,anchor:'center'} },
  { from:'requester',to:'request',pts:[[887,614],[887,550],[837,550],[837,479]],label:{s:'al',x:904,y:575,t:'依賴',rot:-90,anchor:'center'} },
  { from:'requester',to:'response',pts:[[1244,614],[1244,550],[1154,550],[1154,479]],label:{s:'al',x:1261,y:575,t:'依賴',rot:-90,anchor:'center'} },
  { from:'requester',to:'transport',pts:[[1023,694],[1023,770],[730,770],[730,774]],label:{s:'al',x:747,y:735,t:'注入依賴',rot:-90,anchor:'center'} },
  { from:'transport',to:'response',pts:[[730,774],[730,715],[1154,715],[1154,479]],label:{s:'al',x:1171,y:652,t:'依賴',rot:-90,anchor:'center'} },
  { from:'requester',to:'url-request',pts:[[1023,694],[1023,870],[315,870],[315,929]],label:{s:'al',x:332,y:841,t:'Foundation mapping',rot:-90,anchor:'center'} },
...
const TEXTS = [
  ...
  {s:'sub',x:110,y:118,t:'auth opt-in：HTTPClient.execute(_:auth:) 驅動 AuthFlow → Requester → injected Transport → raw HTTPResponse'},
  {s:'tag',x:110,y:146,runs:[{t:'HTTPURL',fill:planeColor('contracts')},{t:' → ',fill:'#4A5462'},{t:'HTTPRequest',fill:planeColor('contracts')},{t:' → ',fill:'#4A5462'},{t:'Auth／AuthFlow',fill:planeColor('contracts')},{t:' → ',fill:'#4A5462'},{t:'HTTPClient',fill:planeColor('client')},{t:' → ',fill:'#4A5462'},{t:'Requester',fill:planeColor('client')},{t:' → ',fill:'#4A5462'},{t:'Transport',fill:planeColor('transport')}]},
  ...
  {s:'bn',x:110,y:1060,t:'不變量：auth overload generic 地送出 flow action 並回灌 raw response；flow 保有認證決策，HTTP status 維持 raw response'},

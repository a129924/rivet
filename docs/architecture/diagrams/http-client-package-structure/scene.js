...
const BANDS = [
  ...
  { id:'band-client', plane:'client', x:110, y:665, w:1260, h:140, alpha:0.5, dash:true,
    hdr:{x:140,y:695,t:'用戶端抽象'}, tagr:{x:1340,y:695,t:'已實作 · 型別化失敗',alpha:0.6} },
  { id:'band-transport', plane:'transport', x:110, y:825, w:1260, h:135, alpha:0.5, dash:true,
    hdr:{x:140,y:855,t:'Transport 邊界'}, tagr:{x:1340,y:855,t:'注入式合約',alpha:0.6} },
  { id:'band-outside', plane:'outside', x:110, y:980, w:1260, h:135, alpha:0.48,
    hdr:{x:140,y:1010,t:'外部 — Foundation 表層'}, tagr:{x:1340,y:1010,t:'非 Rivet 所有',alpha:0.6} }
];
...
const BOXES = [
  ...
  { id:'request-authorization', plane:'contracts', band:'band-contracts', x:140,y:558,w:545,h:58,r:10,dash:true,
    name:'RequestAuthorization',about:'呼叫端在 bare execution 前同步套用的 HTTPRequest 轉換；不傳送請求。',
  ...
  { id:'async-request-authorization', plane:'contracts', band:'band-contracts', x:706,y:558,w:613,h:58,r:10,dash:true,
    name:'AsyncRequestAuthorization',about:'呼叫端在 bare execution 前非同步套用的 HTTPRequest 轉換；一般 error 原樣交回呼叫端。',
  ...
  { id:'http-client', plane:'client', band:'band-client', x:140,y:720,w:360,h:68,r:10,dash:true,
    ...
    texts:[['bl',164,748,'HTTPClient／Configuration'],['bs',164,770,'絕對 URL／相對路徑 → execute(_:)']] },
  { id:'auth-requester', plane:'client', band:'band-client', x:540,y:720,w:360,h:68,r:10,dash:true,
    ...
    texts:[['bl',564,748,'AuthRequester'],['bs',564,770,'目標：保有原始請求；解讀語意決策']] },
  ...
  { id:'url-request', plane:'outside', band:'band-outside', x:140,y:1035,w:350,h:68,r:10,
    ...
    texts:[['bl',164,1063,'Foundation URLRequest'],['bs',164,1085,'Requester 的輸出表層']] },
  ...
  { id:'urlsession', plane:'outside', band:'band-outside', x:920,y:1035,w:399,h:68,r:10,
    ...
    texts:[['bl',944,1063,'Foundation URLSession'],['bs',944,1085,'由套件擁有的 Transport 採用']] }
];
...
const EDGES = [
  ...
  { from:'auth-requester',to:'response',pts:[[860,714],[860,700],[1154,700],[1154,479]],label:{s:'al',x:1007,y:688,t:'策略輸入',anchor:'center'} },
  ...
  { from:'requester',to:'url-request',pts:[[1129,794],[1129,970],[315,970],[315,1029]],label:{s:'al',x:332,y:941,t:'Foundation 對映',rot:-90,anchor:'center'} },
  ...
  { from:'transport',to:'urlsession',pts:[[1120,954],[1120,970],[1120,1029]],label:{s:'al',x:1137,y:992,t:'正式執行',rot:-90,anchor:'center'} }
];

const TEXTS = [
  {s:'title',x:110,y:86,t:'RivetHTTPClient — 設定式介面結構'},
  {s:'sub',x:110,y:118,t:'呼叫端請求授權轉換 → 原始執行；Auth 區塊標示既有模型 A 與已採用、尚未實作的模型 C'},

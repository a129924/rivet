...
const BOXES = [
  ...
  { id:'http-client', plane:'client', band:'band-client', x:140,y:620,w:360,h:68,r:10,dash:true,
    ...
    texts:[['bl',164,648,'HTTPClient／Configuration'],['bs',164,670,'absolute URL／relative path → execute(_:)']] },
...
const EDGES = [
  ...
  { from:'http-client',to:'requester',pts:[[505,640],[925,640],[925,642],[934,642]],label:{s:'al',x:722,y:628,t:'configured execute',anchor:'center'} },
  ...
  { from:'auth-requester',to:'request',pts:[[780,614],[780,580],[837,580],[837,479]],label:{s:'al',x:800,y:568,t:'依賴',anchor:'center'} },
  { from:'auth-requester',to:'response',pts:[[860,614],[860,600],[1154,600],[1154,479]],label:{s:'al',x:1007,y:588,t:'依賴',anchor:'center'} },
...
const TEXTS = [
  ...
  {s:'sub',x:110,y:118,t:'Configuration → URL validation／composition → raw execution；internal AuthRequester 注入 Requester 與 Auth'},
  ...
  {s:'bn',x:110,y:1040,t:'不變量：Configuration 提供 generic base URL／relative path convenience，不是 Endpoint 或 Query builder'},
  {s:'bn',x:110,y:1060,t:'不變量：HTTPURL validation 與 HTTPClientError 分別表達 configuration／URL／path validation、transport failure；HTTP status 維持 raw response'},

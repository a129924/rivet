...
const BANDS = [
  ...
  { id:'band-client', plane:'client', x:160, y:760, w:1260, h:156, alpha:0.5, dash:true,
    hdr:{x:184,y:790,t:'CLIENT ABSTRACTIONS'}, tagr:{x:1396,y:790,t:'已實作 · typed failures',alpha:0.6} },
...
const BOXES = [
  ...
  { id:'transport', plane:'transport', band:'band-transport', x:200,y:1060,w:1220,h:68,r:10,dash:true,
    ...
    texts:[['bl',224,1088,'Transport／URLSessionTransport'],['bs',224,1110,'HTTPClientError；raw HTTP status']] },
  ...
  { id:'urlsession', plane:'outside', band:'band-outside', x:980,y:1300,w:440,h:68,r:10,
    ...
    texts:[['bl',1004,1328,'Foundation URLSession'],['bs',1004,1350,'由 package-owned transport 採用']] }
];
...
const EDGES = [
  ...
  { from:'transport',to:'urlsession',pts:[[1180,1134],[1180,1200],[1200,1200],[1200,1294]],label:{s:'al',x:1214,y:1214,t:'production execution',rot:-90,anchor:'center'} }
];

const TEXTS = [
  {s:'title',x:160,y:86,t:'RivetHTTPClient — 最小介面結構'},
  {s:'sub',x:160,y:118,t:'呼叫端提供已驗證 HTTPURL → HTTP method facade → execute(_:) → Requester → injected Transport／URLSessionTransport → HTTPResponse；decoder 由 caller 擁有'},
  ...
  {s:'bn',x:160,y:1550,t:'不變量：HTTPURL validation 與 HTTPClientError 分別表達 URL construction、transport failure；HTTP status 維持 raw response'},
  {s:'bn',x:160,y:1570,t:'延後：Endpoint、status／Content-Type validation、retry、token refresh 與 package-owned decoder'}
];
const SWATCHES = [

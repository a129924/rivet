...
const BOXES = [
  ...
  { id:'auth-requester', plane:'coordination', band:'band-coordination', x:180,y:466,w:530,h:86,r:10,dash:true,
    name:'AuthRequester',about:'保有呼叫端原始請求並解讀流程的語意決策，不重訂認證策略。',
    texts:[['bl',204,494,'AuthRequester'],['bs',204,516,'原始請求唯一所有者；語意解讀者'],['bn',204,538,'只委派輸入／輸出；不擁有重試策略']] },
  { id:'request-instance', plane:'coordination', band:'band-coordination', x:770,y:466,w:530,h:86,r:10,dash:true,
    name:'保留的 HTTPRequest',about:'由 AuthRequester 以保留的呼叫端意圖準備的選定請求；裝飾表示法尚未鎖定。',
    texts:[['bl',794,494,'保留的 HTTPRequest'],['bs',794,516,'保留呼叫端意圖；裝飾表示法待定'],['bn',794,538,'不是 AuthFlow 可任意建構的請求']] },
...
const EDGES = [
  ...
  { from:'auth-requester',to:'request-instance',pts:[[716,509],[764,509]],label:{s:'al',x:740,y:497,t:'保留呼叫端意圖',anchor:'center'} },
  { from:'request-instance',to:'requester',pts:[[1035,558],[1035,930],[353,930],[353,1020]],label:{s:'al',x:1052,y:744,t:'選定請求',rot:-90,anchor:'center'} },
...
const TEXTS = [
  ...
  {s:'bn',x:110,y:1406,t:'歷史：現行 `.send(HTTPRequest)` 任意請求能力是既有模型 A 基線，不是本圖目標。'},

...
- actor private state 擁有完整 credential、current snapshot、published version、single in-flight restore/refresh task 與 persist-failure unavailable state。
- 第一個成功 publish 的 snapshot version 是 `1`；每次 accepted rotation 已成功 persist 後才遞增。version 僅在單一 provider instance 有效。
...
- fetcher 成功回傳 bundle 代表 rotation 已 accepted。provider 先 `store.save`，成功後才取代 private credential、遞增 version、publish snapshot。

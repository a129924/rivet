...
- `TokenSnapshot.version` 只用於 equality 與 staleness。一般無 expiry race 時首個 delivered snapshot 是 v1；若首次 restore 的 credential 已 expired，且首次 persisted rotation 又已 expired，provider 必須再 refresh，首個 delivered snapshot 可以是 v2。

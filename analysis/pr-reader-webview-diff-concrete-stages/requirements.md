...
- 以 immutable per-file input VO 與 internal `GitDiffTemplate` 建立單檔 unified Git diff source。
...
- Validator 解析 unified diff syntax，或 Parser／Renderer 修改 snapshot。
...
- Template／Parser：added、removed、modified、renamed patch；no-patch metadata entry；不合法 diff 與 dependency exception 為 `parse-error`。

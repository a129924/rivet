...
兩圖的虛線都代表 Rivet 擁有、尚未實作的抽象；實線代表可替換的 Foundation／外部 surface。scene 不得將構想畫成已實作 API，且不得表達 runtime sequence、錯誤種類、重試或 token refresh policy。

## Written

- 本 topic 的四份 artifacts。
- 子 package manifest、`Sources/.gitkeep`、`Tests/.gitkeep`。
- 兩個 diagram folder 的 `scene.js` 與 generated `index.html`。

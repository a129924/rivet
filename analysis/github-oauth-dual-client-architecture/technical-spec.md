...
以文件鎖定既有 GitHub OAuth credential 的雙 Client token lifecycle ownership，讓未來 REST 與 Apollo GraphQL infrastructure 可共享一份 token state，而不把 GitHub OAuth policy 放進 generic HTTP foundation。
...
- Facade 只作 composition：建立單一共享 `OAuthTokenProvider` instance，並注入兩個 client；Facade 不預先替每一個工作驗證 token。
...
| GitHub REST client | 取得 snapshot、加入 Bearer、送出與重送自己的 HTTPRequest 一次 | OAuth／Keychain／refresh-token knowledge |
...
6. Fetcher 成功後，Provider 保存完整 rotated credential bundle、更新 memory snapshot/version，再讓等待者取得新版 snapshot。
7. 原 REST request 或原 Apollo operation 由原 client 以新版 snapshot 重送一次。每個原工作最多一次 recovery retry。
8. 無 credential、permanent refresh failure 或 retry 後第二次 401，產生 authentication-required／重新登入 outcome；transient refresh failure 不清除 credential，回 technical failure。
...
- **TC-04**：lifecycle 圖呈現 permanent authentication-required、transient technical failure，及 403 no-refresh boundary。
- **TC-05**：Phase 2 artifacts 只在 Plan Review approved 後產生，並通過對應 diagram validation。

...
- 記錄 TokenStore、Fetcher、Provider、TokenSnapshot、REST request 與 Apollo operation 的責任與所有權；本 topic 不取代既有 `GitHubTokenProvider`／Store boundary，亦不定義 migration。
- 記錄 REST client 取得 bare HTTP sender／request executor 與 TokenProvider；它保有並重送自己的 request。
- 記錄 expiry-valid snapshot、snapshot version、restore、expiry、rotation、single-flight、一次 401 retry 與 permanent／transient failure boundary。401 是唯一觸發此 recovery 的 response status；second 401 為 terminal authentication-required。transient technical failure 的「保留 credential」僅適用於尚未接受遠端 rotation 的失敗；遠端 rotation 已接受後的本地 persistence failure 必須以可見 technical failure 呈現，不宣稱舊 bundle 仍可用，reconciliation 留待後續 topic。
- 記錄 REST 與 GraphQL 不互相呼叫；GraphQL 不經 REST route；Provider 不持有、不接收或重送 request／operation。
- 保留 v1／v2／v3／v4／v5 lifecycle artifacts 作 immutable rejected evidence，並新增 canonical manifest 指出已驗證交付的繁體中文 v6 為本 topic 的 canonical lifecycle；v6 表達 expiry-valid snapshot、401 recovery、second-401 terminal 與 post-rotation persistence visible/deferred outcome。責任／依賴 canvas 僅修正 colour，並明示 GitHub REST client → bare HTTP sender／request executor edge；不改變其他責任、依賴或 runtime-sequence boundary。
...
5. 現有 `GitHubTokenProvider`／Store boundary 不被本 topic 取代，亦無 migration；v1／v2／v3／v4／v5 lifecycle artifacts 保留為 immutable rejected evidence，canonical manifest 指出已驗證交付的繁體中文 v6 為 canonical lifecycle；v6 明確表達 expiry-valid snapshot、second-401 terminal、post-rotation persistence visible/deferred outcome。
...
- **TC-04**：文件明示 expiry-valid snapshot、同版本 concurrent 401 single-flight、401-only recovery trigger、每個原工作最多 retry 一次、second 401 terminal，且 403 不 refresh。
- **TC-05**：文件區分 authentication-required（無 credential、permanent refresh failure、第二次 401）與 transient technical failure；僅 pre-rotation technical failure 保留 credential，post-rotation persistence failure 是 visible technical failure、defer reconciliation 且不宣稱舊 bundle 可用。
...
- **TC-07**：v1／v2／v3／v4／v5 lifecycle artifacts 保留為 immutable rejected evidence；canonical manifest 指出已驗證交付的繁體中文 v6 為 canonical lifecycle，且 v6 包含 expiry-valid snapshot、second-401 terminal 與 post-rotation persistence visible/deferred outcome；canvas colour 修正與 REST → bare executor edge 不改其他語意。

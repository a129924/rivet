...
- 將正式 topic 的唯一 artifacts 固定為同 slug 的 `requirements.md`、`technical-spec.md`、`.plan.md` 與 `.step.md`。
...
- 提供 branch 命名、完整且可操作的 worktree lifecycle，以及 human-confirmed commit 的 Git workflow 指引。
...
- `plan-step-tracker` 能發現缺少 owner role、驗證證據、blocker 或 human-check 的 `.step.md`，但不產生 verdict 或放行流程。
